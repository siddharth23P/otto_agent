...
- Issue #1. Every `execute_python` call was a standalone script in a fresh process, so an agent that loaded a dataset or built an index had to rebuild it on the next call, from the transcript or through disk; Prime Agent (arXiv:2608.23552) names the persistent interpreter as the layer that makes long-horizon work compositional. One interpreter per run now, behind the same contextvar seam as the workspace and the command runner, started on the first call and closed with the run. Written tests-first against the three forks the issue asked to settle: a delegated subagent gets its own session, never the parent's; inside a container the tool keeps its one-shot script and says so; a call the interrupt cannot stop is killed and the result says the session restarted. The first run of the suite found the plumbing's own dependency on the builtins: a snippet that rebound `len` broke the shim's `json.loads`, and every call after it timed out silently; builtins are now restored after each call. The acceptance measurement the issue names, Claw-Eval score and `model_calls` with the session on and off (`OTTO_PYTHON_SESSION=0`), needs keys and is not taken here. **1,593 tests.**
- `80e8212` The ask was an Android app that does what a person asks on their own phone -- change a setting, install an app, find milk on Blinkit and stop at the payment page -- with Otto as its brain, running on the phone itself. Otto had no way to be a library: `~/.otto` and the repository `.env` were constants, `Router()` was built at import, and every front end was a terminal. `OTTO_HOME`/`OTTO_ENV_FILE` (agent/config/home.py) move the state; `agent/embed.py` is the surface a host depends on (`configure`, keys with a keystore mode that never writes a file, `Runtime`, `SessionHandle.run` with JSON events, `answer`/`cancel` from another thread), versioned by `API_VERSION`. A tool profile takes the shell, browser and desktop tools off a phone's menu; guidance rides with a bound toolkit. `agent/phone/` is the device-agnostic half of the app: a `PhoneBackend` Protocol, a bounded inert screen digest that never shows a password field, the money guard as package data with two-signal sensitive-screen verdicts (a chat mentioning an OTP is not a payment screen; an input field asking for one is), and eight tools of which only `phone_install` and `phone_commit` are mutating, so the gate holds an install once per package and never a tap. `otto serve` puts the same runtime behind a WebSocket with a token and a versioned protocol, for a laptop during app work and for a Linux userland on the phone. Dependencies were reshaped for Android wheels: fastembed and langgraph-cli became extras, a vendor whose SDK is missing disables that vendor rather than the router. Measured on the way: the seven compiled packages with no Android wheel today (pydantic-core, jiter, orjson, ormsgpack, tiktoken, uuid-utils, zstandard) are the app repository's wheel-building spike, not this package's problem. Version 0.2.0. **1,635 tests.**
- Three reviews of that PR landed as it merged, two of them with findings on the money guard: `pay_words` was a phrase list, so a button labelled just "Pay" or "Buy" matched nothing; `press enter` (the keyboard's submit) never consulted the guard; a tap by coordinates on a point with no element under it fell through unjudged; a Cyrillic а in "Pаy now" defeated the word list. All four are closed with the rules as data: the bare words are pay words matched whole, forward words ("Continue", "Next") become pay words next to a checkout signal (a total, "payment", a card field), "Checkout" is a commit word so reaching the payment page is the person's call once, `enter` is judged like a tap, a blind tap is refused on a screen that has elements, and `normal()` folds Cyrillic and Greek look-alikes to Latin. Around it: `all_models()` skips a vendor whose SDK is missing, keys injected through `configure(environ=...)` switch the subprocess tools off by default, the env file is made owner-only on every write, `otto serve` refuses a browser origin before the handshake and runs turns on its own bounded pool, `configure(strict=True)` raises on a late import and sets `OTTO_OUTPUT_DIR`. **1,715 tests.**
